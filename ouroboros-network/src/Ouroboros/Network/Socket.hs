...
beginConnection fn addr st = do
    accept <- fn addr st
    case accept of
      AcceptConnection st' app -> pure $ Server.Accept st' $ \sd ->
        case app of
...
withServerNode addr fn k =
    bracket (mkListeningSocket (Socket.addrFamily addr) (Just $ Socket.addrAddress addr)) Socket.close $ \sd -> do
      ...
      withAsync
        (runNetworkNode'
          sd throwIO
          (\addr st -> pure $ AcceptConnection st (fn addr))
          complete
          main
          ()) k
...
withSimpleServerNode
    ...
    -- throws an exception.
    -> IO t
withSimpleServerNode addr app k = withServerNode addr (\_ -> AnyMuxServerApp app) k
