...
--
prop_channel_ST :: (Tx -> Bool) -> [Tx] -> Bool
...
--
prop_channel_IO :: (Tx -> Bool) -> [Tx] -> Property
...
--
prop_pipe_IO :: (Tx -> Bool) -> [Tx] -> Property
prop_pipe_IO p txs =
...
instance Arbitrary (AnyMessageAndAgency (LocalTxSubmission Tx)) where
  arbitrary = oneof
...
instance Show tx => Show (AnyMessageAndAgency (LocalTxSubmission tx)) where
  show (AnyMessageAndAgency _ msg) = show msg

instance Eq tx => Eq (AnyMessage (LocalTxSubmission tx)) where
  ...
  (==) (AnyMessage MsgRejectTx)
       (AnyMessage MsgRejectTx) = True
...
--
prop_codec :: AnyMessageAndAgency (LocalTxSubmission Tx) -> Bool
prop_codec msg =
  runST (prop_codecM codec msg)

-- | Check for data chunk boundary problems in the codec using 2 chunks.
--
prop_codec_splits2 :: AnyMessageAndAgency (LocalTxSubmission Tx) -> Bool
...
--
prop_codec_splits3 :: AnyMessageAndAgency (LocalTxSubmission Tx) -> Bool
