...
instance Protocol (TxSubmission txid tx) where
  ...
  data Message (TxSubmission txid tx) from to where
    ...
    -- can be accepted in the response. This must be greater than 0. The
    -- number requested must not put the total outstanding over the fixed
    -- protocol limit.
    --
    ...
    MsgRequestTxIds
      ...
      -> Word16 -- ^ Request up to this number of txids. Must be @> 0@.
