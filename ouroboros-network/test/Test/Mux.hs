...
failingServer  = do
    ...
    (_, _, serverApp) <- setupMiniReqRsp (return ()) endMpsVar request response
    serverAsync <- startMuxSTM (Mx.MuxServerApplication $ \ChainSync1 -> serverApp) server_w server_r sduLen Nothing
